...
def email_settings():
    ...
    from the admin tool's invite commands (diytracker-admin repo), which run
    outside a request, so services/mail.py must not depend on an app context
    to reach them. Kept here anyway so every os.environ read in the app still
    lives in one file.
    """
    return (
