...
class TestIngestCore:
    ...
    def test_flyer_bytes_stored(self, app):
        result = ingest_event(_payload(), flyer=(_png_bytes(), 'flyer.png'),
    ...
    def test_non_image_flyer_rejected(self, app):
        result = ingest_event(_payload(), flyer=(b'#!/bin/sh\nrm -rf /', 'evil.png'),
                              upload_folder=app.config['UPLOAD_FOLDER'])
        assert result.status == 'invalid'
...
class TestIngestApi:
    ...
    def test_duplicate_push_returns_200(self, client, ingest_token):
        assert client.post('/api/ingest', json=_payload(), headers=_auth()).status_code == 201
        resp = client.post('/api/ingest', json=_payload(), headers=_auth())
        assert resp.status_code == 200
        assert resp.get_json()['status'] == 'duplicate'

    def test_invalid_payload_returns_422(self, client, ingest_token):
        resp = client.post('/api/ingest', json=_payload(start_date=None), headers=_auth())
        assert resp.status_code == 422

    def test_non_object_body_returns_400(self, client, ingest_token):
        resp = client.post('/api/ingest', data='not json', headers=_auth(),
                           content_type='application/json')
        assert resp.status_code == 400

    def test_multipart_push_with_flyer(self, client, app, ingest_token):
        resp = client.post('/api/ingest', headers=_auth(), data={
...
class TestFlyerApproval:
    ...
    def test_queue_shows_flyer_and_submitter(self, client, app, make_user, login):
        ingest_event(_payload(), flyer=(_png_bytes(), 'flyer.png'),
                     upload_folder=app.config['UPLOAD_FOLDER'])
        login(make_user())
...
class TestForwarder:
    ...
    def test_multipart_encoding_accepted_by_endpoint(self, client, app, ingest_token):
        ...
        resp = client.post('/api/ingest', data=body, content_type=content_type,
                           headers=_auth())
        assert resp.status_code == 201
        rec = ScrapedEvent.query.one()
        assert rec.flyer and os.path.exists(rec.flyer)
    ...
    def store(self, tmp_path):
        recs = [
            {'id': 'good00000001', 'submitter': 'Jona',
             'event': {'title': 'Show A', 'date': _future(), 'artists': []}},
            {'id': 'bad000000002', 'submitter': 'Jona',
             'event': {'title': 'Dateless', 'date': '', 'artists': []}},
            {'id': 'good00000003', 'submitter': 'Jona',
    ...
    def _run(self, monkeypatch, store, deliver):
        ...
        monkeypatch.setattr('sys.argv', ['eventbot_forwarder.py', '--store', str(store)])
        return fwd, calls
...
class TestEventbotMapping:
    def test_record_maps_to_payload(self, app):
        ...
        rec = {
            ...
            'event': {
                'title': '1-a-Grind', 'artists': ['Archagathus', 'Skunk'],
                'venue': 'Quai du Bas 30', 'city': 'Biel/Bienne',
                'address': 'Quai du Bas 30, Biel/Bienne',
                'date': _future(), 'start_time': '19:30', 'doors_time': '19:00',
                'price': '', 'ticket_url': '', 'genre': 'grind, crust',
                'age_restriction': '18+', 'description': 'Grind night.', 'links': [],
            },
        }
