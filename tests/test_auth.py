...
class TestLogin:
    def test_valid_login_sets_session(self, client, make_user):
        ...
        resp = client.post('/login', data={'email': 'u@example.com', 'password': 'secret123'})
    ...
    def test_invalid_password_does_not_authenticate(self, client, make_user):
        make_user(email='u@example.com', password='secret123')
        resp = client.post('/login', data={'email': 'u@example.com', 'password': 'nope'}, follow_redirects=True)
        with client.session_transaction() as sess:
            assert 'user_id' not in sess
        assert b'Invalid email or password' in resp.data

    def test_unknown_user(self, client):
        resp = client.post('/login', data={'email': 'ghost@example.com', 'password': 'x'}, follow_redirects=True)
        with client.session_transaction() as sess:
            assert 'user_id' not in sess
        assert b'Invalid email or password' in resp.data
    ...
    def test_login_redirects_to_safe_next(self, client, make_user):
        make_user(email='u@example.com', password='secret123')
        resp = client.post('/login?next=/queue', data={'email': 'u@example.com', 'password': 'secret123'})
...
class TestAccessControl:
    def test_login_required_redirects_anonymous(self, client):
        ...
        assert 'next=%2Fsubmit' in resp.headers['Location'] or 'next=/submit' in resp.headers['Location']
