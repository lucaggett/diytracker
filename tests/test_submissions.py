...
class TestSubmitEvent:
    ...
    def test_submit_reuses_existing_venue(self, client, make_user, login, make_venue):
        ...
        client.post('/submit', data={
            'name': 'Reuse Show', 'date': _future(), 'doors': '21:00',
            'ticket_price': '10', 'venue_id': str(venue.id),
        })
    ...
    def test_new_venue_missing_details_is_rejected(self, client, make_user, login):
        login(make_user())
        resp = client.post('/submit', data={
            'name': 'Bad', 'date': _future(), 'doors': '20:00', 'ticket_price': '5',
            'venue_id': 'new', 'venue_name': 'No City Venue',  # missing city + plz
...
class TestEventQueue:
    def _make_scraped(self, **kw):
        defaults = dict(
            source='metalgigs', url='https://metalgigs.ch/konzerte/show-1',
            title='Scraped Show', performers='Headliner',
            styles='Metal', start_date=date.today() + timedelta(days=20),
            doors_open=time(19, 0), venue_name='Hall', city='Aarau',
            postal_code='5000', region='AG', approved=False,
        )
        defaults.update(kw)
    ...
    def test_duplicate_event_is_rejected_by_hash(self, client, admin, login):
        ...
        common = dict(title='Same Gig', styles='Punk', venue_name='Hall',
    ...
    def test_source_filter(self, client, admin, login):
        login(admin)
        self._make_scraped(title='MG', source='metalgigs',
                           url='https://metalgigs.ch/konzerte/a')
        self._make_scraped(title='Petzi', source='petzi',
                           url='https://petzi.ch/en/events/b')
