...
def main():
    ...
    ap.add_argument('--store', default=os.path.join('instance', 'eventbot'),
    ...
    if not os.path.exists(jsonl):
        sys.exit(f'No events.jsonl in {args.store!r} — rsync the store first (see docstring).')
        ...
        with open(jsonl, encoding='utf-8') as f:
            for line in f:
                ...
                if rec.get('flyer_image') and flyer is None:
                    missing_flyers += 1
                result = ingest_event(eventbot_to_payload(rec), flyer=flyer, commit=False)
                counts[result.status] += 1
                if result.status == 'invalid':
                    print(f"  skipped {rec.get('id')}: {result.reason}")
        db.session.commit()

    print(f"Imported {counts['created']} new events "
          f"({counts['duplicate']} already known, {counts['invalid']} invalid).")
    if missing_flyers:
        print(f"Warning: {missing_flyers} records referenced flyers missing from {flyers_dir!r}.")
