...
def main():
    ...
    if not token and not args.dry_run:
        sys.exit('Set INGEST_TOKEN in the environment (same value as on the diytracker server).')
    ...
    for rec in records:
        ...
        if args.dry_run:
            print(f'would send {rec_id}: {payload.get("title")!r}'
        ...
        if status in (200, 201):
            ...
            skipped += 1
            print(f'{rec_id}: rejected as invalid ({detail.get("error")}), will not retry',
                  file=sys.stderr)
        else:
            ...
            sys.exit(f'{rec_id}: HTTP {status} {detail} — aborting, will retry next run.')

    if not args.dry_run:
        with open(state_path, 'w', encoding='utf-8') as fh:
            json.dump(forwarded, fh, indent=1)
    print(f'Done: {sent} delivered, {skipped} rejected, '
