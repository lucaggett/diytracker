...
with app.app_context():
    with db.engine.connect() as conn:
        existing = {row[1] for row in conn.execute(db.text('PRAGMA table_info(scraped_event)'))}
        for name, ddl_type in COLUMNS:
            if name in existing:
                print(f'  column {name} already present, skipping')
                continue
            conn.execute(db.text(f'ALTER TABLE scraped_event ADD COLUMN {name} {ddl_type}'))
